...
for s = 1:nDo
    ...
    % urchanlocs captured NOW (full montage with positions) so that ANY
    % channel removed later -- by clean_rawdata OR by the manual override
    % below -- gets correctly interpolated back at the end of this script.
    EEG.urchanlocs = EEG.chanlocs;

    % ---- manual per-subject bad-channel override (see config.m) ----
    if isKey(cfg.manual_bad_channels, subj)
        badChans = cfg.manual_bad_channels(subj);
        fprintf('  Manually excluding %d channel(s) before cleaning (QC override): %s\n', ...
            numel(badChans), strjoin(badChans, ', '));
        keepChans2 = setdiff({EEG.chanlocs.labels}, badChans, 'stable');
        EEG = pop_select(EEG, 'channel', keepChans2);
    end
    ...
    if ~isempty(interp_channels)
        ...
        EEG = pop_select(EEG, 'nochannel', interp_channels);  % actually remove so pop_interp() below rebuilds them
    end
